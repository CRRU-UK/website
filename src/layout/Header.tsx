...
const researchSubmenu = {
  ...
  items: [
    ...
    { title: 'Photo-ID Archives', path: sitemap.catalogues.path },
...
const rescueSubmenu = {
  ...
  items: [
    ...
    { title: 'Species Identification Key', path: sitemap['species-identification-key'].path },
  ],
};
...
const trainingSubmenu = {
  ...
  items: [
    ...
    { title: 'Summer Training Placements', path: sitemap['summer-training-placements'].path },
    { title: 'MMO Training Courses', path: sitemap['mmo-training'].path },
    { title: 'CPD PAM Training Courses', path: sitemap['pam-training'].path },
    { title: 'Gain University Credits', path: sitemap['gain-university-credits'].path },
  ],
};
    ...
    <header>
      ...
      <nav>
        <ul className="menu">
          <li className="dropdown">
            <Link href={sitemap.about.path} onClick={(event) => handleSubmenuOpen(event, Submenus.About)}>
              <span>About Us</span>
            </Link>
            <SubmenuItem active={submenu === Submenus.About} data={aboutSubmenu} />
          </li>
          <li className="dropdown">
            <Link href={sitemap.research.path} onClick={(event) => handleSubmenuOpen(event, Submenus.Research)}>
              <span>Research</span>
            </Link>
            <SubmenuItem active={submenu === Submenus.Research} data={researchSubmenu} />
          </li>
          <li className="dropdown">
            <Link href={sitemap.rescue.path} onClick={(event) => handleSubmenuOpen(event, Submenus.Rescue)}>
              <span>Rescue</span>
            </Link>
            <SubmenuItem active={submenu === Submenus.Rescue} data={rescueSubmenu} />
          </li>
          <li className="dropdown">
            <Link href={sitemap.education.path} onClick={(event) => handleSubmenuOpen(event, Submenus.Education)}>
              <span>Education</span>
            </Link>
            <SubmenuItem active={submenu === Submenus.Education} data={educationSubmenu} />
          </li>
          <li className="dropdown">
            <Link href={sitemap.training.path} onClick={(event) => handleSubmenuOpen(event, Submenus.Training)}>
              <span>Training</span>
            </Link>
            <SubmenuItem active={submenu === Submenus.Training} data={trainingSubmenu} />
          </li>
          <li className="dropdown">
            <Link href={sitemap.help.path} onClick={(event) => handleSubmenuOpen(event, Submenus.Help)}>
              <span>Help</span>
            </Link>
      ...
      >
        <svg fill="none" height="48" viewBox="0 0 64 48" width="64" xmlns="http://www.w3.org/2000/svg" className="burger-icon">
          ...
          <path d="m2 24c2.03077 2.1662 5.46923 2.1662 7.5 0 2.0308-2.1662 5.4692-2.1662 7.5 0 2.0308 2.1662 5.4692 2.1662 7.5 0 2.0308-2.1662 5.4692-2.1662 7.5 0 2.0308 2.1662 5.4692 2.1662 7.5 0 2.0308-2.1662 5.4692-2.1662 7.5 0 2.0308 2.1662 5.4692 2.1662 7.5 0 2.0308-2.1662 5.4692-2.1662 7.5 0" stroke="#fff" strokeLinecap="round" strokeWidth="3" />
          <path d="m19.845 25.2345c7.4508 3.474 7.9963 8.1523 7.5342 11.7654l9.2795.0001c-.4296-3.3798.2163-7.7552 7.5865-11.7654 11.2513-6.1371 8.5745-18.75909 8.241-21.48225-.2813-2.32254-1.4105-2.0334-2.5397-.8754-4.0957 5.64648-14.0742.98317-17.9465 9.12305-3.871-8.13997-13.7689-3.47666-17.8562-9.12314-1.1292-1.15791-2.2584-1.44714-2.5482.8754-.3321 2.72325-3.34951 16.06554 8.2494 21.48224z" fill="#0060c2" id="whale" />
